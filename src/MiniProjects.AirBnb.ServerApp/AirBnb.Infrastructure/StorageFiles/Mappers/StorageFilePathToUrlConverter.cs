...
/// </summary>
public class StorageFilePathToUrlConverter(IOptions<StorageFileSettings> storageFileSettings, IOptions<ApiSettings> apiSettings)
    : IValueConverter<StorageFile, string>
{
    public string Convert(StorageFile sourceMember, ResolutionContext context)
    {
        var test = storageFileSettings.Value.LocationSettings.ToList();
